...
def main():
    ...
    services = [
        ...
        {"name": "Keywords & Intentions (Module A)", "script": "server_keywords.py", "port": 8003},
    ]
    
    for s in services:
        print(f"--> Démarrage de {s['name']} sur http://127.0.0.1:{s['port']} (Swagger Docs: http://127.0.0.1:{s['port']}/docs)...")
        p = subprocess.Popen([python_bin, s["script"]])
        processes.append(p)
        time.sleep(1)
        
    print("\n" + "=" * 60)
    print("✅ TOUS LES 4 MICROSERVICES SONT EN COURS D'EXÉCUTION !")
    print("=" * 60)
    print("  • Hub Dashboard (Accueil)     : http://127.0.0.1:8002")
    print("  • Keywords & Intents (Mod A)  : http://127.0.0.1:8003 (Docs: /docs)")
    print("  • People Also Ask + IA (Mod C): http://127.0.0.1:8001 (Docs: /docs)")
    print("  • Pack Factory (Module B)     : http://127.0.0.1:8000 (Docs: /docs)")
    print("=" * 60)
